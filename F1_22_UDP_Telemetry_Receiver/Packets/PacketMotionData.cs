...
namespace F1_22_UDP_Telemetry_Receiver.Packets
{
    ...
    public class PacketMotionData : Packet
    {
        public PacketMotionData(byte[] data) : base(data)
        {
            ...
            suspensionVelocitynew = new float[4];
            for (int i = 0; i < WHEEL_COUNT; i++)
            {
                suspensionVelocitynew[i] = BitConverter.ToSingle(data, byteIndex);
                byteIndex += 4;
            }

            suspensionAccelerationnew = new float[4];
            for (int i = 0; i < WHEEL_COUNT; i++)
            {
                suspensionAccelerationnew[i] = BitConverter.ToSingle(data, byteIndex);
                byteIndex += 4;
            }
            ...
            for (int i = 0; i < WHEEL_COUNT; i++)
            {
                wheelSlip[i] = BitConverter.ToSingle(data, byteIndex);
                byteIndex += 4;
            }
        ...
        public float[] suspensionPosition;// = new float[4];       // Note: All wheel arrays have the following order:
        public float[] suspensionVelocitynew;// = new float[4];       // RL, RR, FL, FR
        public float[] suspensionAccelerationnew;// = new float[4];  // RL, RR, FL, FR
        public float[] wheelSpeed;//= new float[4];              // Speed of each wheel
        public float[] wheelSlip;// = new float[4];                // Slip ratio for each wheel
